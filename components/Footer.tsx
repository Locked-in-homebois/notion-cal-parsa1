        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8">
          ...
          <div>
            <Link href="/" className="flex items-center mb-6">
                <Image src="/Icons/notion-logo.svg" alt="Notion Logo" width={24} height={24} />
                <span className="font-semibold text-lg ml-2">Notion</span>
            </Link>
            <div className="flex space-x-4 mb-6">
              {socialLinks.map((social) => (
                <a key={social.name} href="#" className="text-gray-500 hover:text-gray-700">
                  <Image src={social.icon} alt={social.name} width={20} height={20} />
                </a>
              ))}
            </div>
            <button className="flex items-center text-sm text-gray-500 mb-4">
              <Image src="/Icons/globe.svg" alt="Globe" width={16} height={16} className="mr-2" />
              English (US)
            </button>
            <p className="text-sm text-gray-500 mb-2">Cookie settings</p>
            <p className="text-sm text-gray-500">&copy; 2025 Notion Labs, Inc.</p>
          </div>
            ...
            <div key={column.title}>
              ...
              <ul className="space-y-3">
                {column.links.map((link) => (
                 ...
                 {column.specialLink && (
                  <li className="mt-4">
                    <a href="#" className="font-bold text-sm text-gray-800 hover:text-gray-900">
                      {column.specialLink}
                    </a>
                  </li>
                )}
